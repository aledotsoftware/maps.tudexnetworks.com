...
## 2024-05-16 - Accessibility: Restoring focus visibility overridden by glassmorphism UI
**Learning:** Modern design trends like Glassmorphism often use `outline: none` on interactive elements to remove default browser styling, which inadvertently breaks keyboard navigation by removing focus indicators.
**Action:** When auditing or implementing UI components with `outline: none`, explicitly add `:focus-visible` styles with `outline-offset` and a color that matches the design system to restore accessible keyboard navigation without compromising the visual aesthetic for pointer users.
