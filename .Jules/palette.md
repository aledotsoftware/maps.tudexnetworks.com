...
## 2024-05-15 - Accessibility: Focus outlines on Glassmorphism UI
**Learning:** When using Frutiger Aero / Glassmorphism UI styles that typically apply `outline: none`, interactive elements lose keyboard navigation visibility. Also, visually unlabelled search inputs (relying only on placeholders) lack context for screen readers.
**Action:** Always explicitly add a complementary `:focus-visible` rule (e.g., `outline: 2px solid #64FFDA; outline-offset: 2px;`) to interactive elements to maintain keyboard navigation accessibility without breaking the thematic design, and add an explicit `aria-label` to input fields that lack a visible `<label>`.
