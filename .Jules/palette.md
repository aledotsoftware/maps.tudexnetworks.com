...
## 2024-05-03 - Replacing interactive divs with semantic buttons in Leaflet components
**Learning:** Leaflet components like toolbars often use `<div>` wrapping an `<a>` to create buttons, which requires manually adding `role="button"`, `tabindex="0"`, and listening to keyboard events. When converting these to semantic `<button>` elements to improve accessibility, the custom CSS selectors that target `.leaflet-bar a` will no longer apply to the new button, causing severe visual regressions where standard 3D button rendering is applied.
**Action:** Always ensure that when replacing an `<a>` with a `<button>` inside a component like Leaflet, you either apply equivalent inline reset styling (`border: none; background-color: var(--map-btn-bg-color); color: var(--map-btn-color); ...`) or update the existing CSS files so that `.leaflet-bar button` matches `.leaflet-bar a` to preserve the layout and visual integration.
