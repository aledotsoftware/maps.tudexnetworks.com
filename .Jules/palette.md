...
## 2024-05-30 - [Refactoring interactive elements to native buttons]
**Learning:** Refactoring custom tags (`div` with `role="button"`) to native `button` tags is critical for accessibility. However, directly applying `outline: none;` without `:focus-visible` fallback breaks WCAG 2.4.7 focus visible requirements. Furthermore, applying resets directly via `element.style = "..."` fails silently because the DOM `style` object is read-only; it requires `element.style.cssText = "..."`.
**Action:** When adding inline CSS resets for dynamically generated buttons in JS, always use `element.style.cssText` and explicitly avoid setting `outline: 0;` or `outline: none;` to ensure default browser focus outlines are preserved, substituting spacing fixes using `outline-offset` if needed.
