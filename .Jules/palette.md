...
## 2024-05-04 - Accessibility: ARIA labels and tooltips for symbol-only controls
**Learning:** Map controls and similar UI elements utilizing single characters or symbols (like '+', '-', 'N', or custom icons) often lack context for screen readers and sighted users. Just adding a visually identifiable symbol isn't enough for true discoverability. Inputs and selects that rely on placeholders instead of visual labels are also a common accessibility pitfall.
**Action:** Always ensure that icon-only or symbol-only interactive elements (`button`, `a`) include both `aria-label`s for screen readers and `title` attributes for tooltips for sighted users. Ensure inputs and selects without visual `<label>` tags have explicit `aria-label` attributes to provide necessary context.
