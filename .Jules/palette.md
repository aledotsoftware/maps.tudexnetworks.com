...
## 2025-02-20 - Restoring Accessibility in Frutiger Aero UIs
**Learning:** Frutiger Aero / Glassmorphism designs often globally apply `outline: none` to inputs and buttons to maintain their visual aesthetic, which completely breaks keyboard navigation visibility. Additionally, bare inputs without visible labels often lack `aria-label` attributes.
**Action:** Always explicitly add a complementary `:focus-visible` rule (e.g., `outline: 2px solid #64FFDA; outline-offset: 2px;`) to interactive elements to restore keyboard accessibility without breaking the thematic design, and explicitly set `aria-label`s on inputs without `<label>` tags.
