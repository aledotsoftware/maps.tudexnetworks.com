...
## 2026-05-14 - Accessible Glassmorphism Inputs
**Learning:** When using modern UI trends like Frutiger Aero or Glassmorphism, inputs and buttons often have `outline: none` to maintain the aesthetic, making them completely inaccessible to keyboard users. Similarly, custom search inputs without visual labels require `aria-label` and `title` to be discoverable.
**Action:** Always ensure custom inputs in these design systems include `aria-label`/`title` and explicitly define a `:focus-visible` state (e.g., using a themed `outline` color) to restore keyboard accessibility without breaking the intended default visual style.
