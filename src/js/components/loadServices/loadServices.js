  ...
  `;
    ...
    if (Object.keys(servicesLoaded)) {
      for (let sourceId in servicesLoaded) {
        ...
        checkbox.checked =
          layerCount === Object.keys(servicesLoaded[serviceID].layers).length;
...
async function handleURLInput(e) {
  ...
  serviceLayer
    .loadWMS(url)
    .then((layers) => {
      if (document.getElementById("wrongURL"))
        document.getElementById("wrongURL").style.display = "none";
      if (document.getElementById("buttonEnd"))
	...
	`;
      ...
      // Add the container to the modal
      document
        .getElementById("select-layers-container")
        .prepend(wmsResultContainer);

      new UserMessage(
        "El servicio fué agregado exitosamente",
        true,
        "information",
      );
      loadingBtn("off", "buttonConectar", "Conectar");
    ...
    .catch((error) => {
      ...
      document.getElementById("wrongURL").innerHTML =
        `<strong>${error.message}</strong>${customErrorMsg}`;
...
function saveServiceTitle(serviceID, event) {
  ...
  let value = input.value.length
    ? input.value
    : servicesLoaded[serviceID].title;
    ...
    for (let layer_name in servicesLoaded[e.target.value].layers) {
      ...
      if (!exist) {
        ...
        menu_ui.addLayerToGroup(
          servicesLoaded[layersIndex[layer_name]].title,
          "WMS",
          layer_name,
          layersIndex[layer_name],
          layer_name,
          servicesLoaded[layersIndex[layer_name]].layers[layer_name],
        );
      }
    }
  } else {
...
function handleLayerCheck(e) {
  if (e.target.checked) {
    ...
    menu_ui.addLayerToGroup(
      servicesLoaded[layersIndex[e.target.value]].title,
      "WMS",
      e.target.value,
      layersIndex[e.target.value],
      e.target.value,
      servicesLoaded[layersIndex[e.target.value]].layers[e.target.value],
    );
  } else {
    addedLayers.forEach((layer) => {
