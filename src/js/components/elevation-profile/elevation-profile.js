...
class IElevationProfile {
      ...
      .then((result) => {
        ...
        for (let i = 0; i < result.coordinates.length; i++) {
          ...
          if (i + 1 < result.coordinates.length) {
            ...
            distancia =
              distancia + turf.distance(desde, hasta, { units: "kilometers" });
          }
        }
        ...
        new UserMessage(
          `Geoproceso ejecutado exitosamente.`,
          true,
          "information",
        );
  ...
  clickDisplayResult(id) {
      ...
      if (lyr.id === id) {
        if (aux.classList.contains("active")) {
          if (wrapper.classList.contains("hidden")) {
            //if wrapper window is closed while btn is active
          ...
          if (wrapper.classList.contains("hidden")) {
            //if wrapper is  hidden & all layers are deactivated
            wrapper.classList.toggle("hidden");
          }
          aux.classList.add("active");
          if (mapa.editableLayers.hasOwnProperty("polyline")) {
            const lyr = mapa.editableLayers["polyline"].find(
              (lyr) => lyr.idElevProfile === id,
            );
            if (lyr) {
    ...
    if (
      document.getElementById("elevationProfile").querySelectorAll(".hidden")
        .length == count
    ) {
      wrapper.classList.toggle("hidden");
    }
  ...
  addWrapper() {
    if (document.getElementById("pt-wrapper")) {
      ...
      wrapper.classList =
        "justify-content-center col-12 col-xs-12 col-sm-10 col-md-10 hidden";
      ...
      s_sec.style =
        "width: 5%; display: flex; justify-content: end; padding-right: 1%;";
          ...
          point: {
            events: {
              mouseOver: function (event) {
                mapa.markerPerfilTopografico = L.circleMarker(
                  [event.target.lng, event.target.lat],
                  {
                    radius: 4,
                    fillOpacity: 0.7,
                    color: polylineColor,
  ...
  getGeoJSON(data) {
    // would be moved to the Layer class as part of export method
  ...
  removeElevationProfile(id) {
    ...
    const polylines = mapa.editableLayers.polyline.filter(
      (lyr) => lyr.idElevProfile === id,
    );
    ...
    let layersCount = addedLayers.filter((layer) =>
      layer.id.includes(this.namePrefixElevProfile),
    ).length;
    let hiddenCount = 0;
